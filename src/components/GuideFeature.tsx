    ...
    <>
      ...
      <section className="w-full px-5 md:hidden">
        <article className="flex flex-col gap-5">
          <div className="relative h-[228px] w-full overflow-hidden rounded-[12px]" style={{ background: '#d8cfc4' }}>
            <Link to={articlePath} aria-label={`Read ${title}`} className="block h-full w-full" prefetch="intent" onPointerEnter={prefetchArticle} onFocus={prefetchArticle}>
          ...
          <div className="flex flex-col gap-4">
            ...
            <Link to={articlePath} className="w-fit transition-opacity hover:opacity-65" prefetch="intent" onPointerEnter={prefetchArticle} onFocus={prefetchArticle}>
            ...
            <Link to={articlePath} className="mt-1 w-full rounded-lg bg-black py-[14px] text-center text-[12px] font-semibold uppercase tracking-[1.2px] text-[#fbf9f7]" prefetch="intent" onPointerEnter={prefetchArticle} onFocus={prefetchArticle}>
              View the guide
            </Link>
          </div>
        </article>
      ...
      <section className="mx-auto hidden w-full max-w-[1440px] px-10 md:block">
        <div className="relative overflow-hidden rounded-lg" style={{ background: '#31353d', aspectRatio: '1344 / 633', minHeight: 560 }}>
          <Link to={articlePath} aria-label={`Read ${title}`} className="absolute inset-0 block" prefetch="intent" onPointerEnter={prefetchArticle} onFocus={prefetchArticle}>
            <img
