...
describe('shared full-bleed hero and CTA layout', () => {
  ...
  it('keeps the approved mobile type scale and supplied favicon wired through the shared templates', () => {
    ...
    expect(header).toContain('helloBarExiting')
    expect(header).toContain("duration-[280ms]")
    expect(header).toContain("document.addEventListener('pointerdown', beginExitBeforeNavigation, true)")
    expect(header).toContain("dataset.ringSocietyHelloBarExiting = 'true'")
    ...
    expect(homeIntro).toContain("dataset.ringSocietyHomeIntroReveal = 'true'")
