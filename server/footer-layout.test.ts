...
describe('footer responsive layout', () => {
  it('uses a deliberate four-column small-laptop grid without changing the mobile stack', () => {
    ...
    expect(footer).toContain("{ label: 'How to Choose A Jeweler: Our 10-Point Framework', to: '/guides/how-to-choose-a-jeweler' }")
    expect(footer).toContain("{ label: 'How to Buy an Engagement Ring: The Definitive Guide', to: '/guides/how-to-buy-an-engagement-ring' }")
    expect(footer).toContain("{ label: 'Engagement Ring Budgets: How Much Should You Spend?', to: '/guides/engagement-ring-budgets' }")
    expect(footer).toContain("{ label: 'Natural vs. Lab-Grown Diamonds: The Honest, Unbiased Comparison', to: '/guides/natural-vs-lab-grown-diamonds' }")
    expect(footer).toContain("{ label: 'The 4Cs of Diamonds Explained in Plain English', to: '/guides/4cs-of-diamonds' }")
