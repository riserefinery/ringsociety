...
describe('CMS page hero settings and contact layout', () => {
  ...
  it('maps page hero focal alignment into the public hero components', () => {
    ...
    expect(read('src/components/Hero.tsx')).toContain('hero-grid-copy')
    expect(read('src/index.css')).toContain('@media (min-width: 1800px)')
    expect(read('src/components/Hero.tsx')).toContain("boxSizing: 'content-box'")
    expect(mission).toContain('imagePosition={imagePosition}')
    expect(mission).toContain("const MISSION_INITIAL_IMAGE_POSITION = 'center bottom'")
    expect(mission).toContain('const imagePosition = pageSettings ? hero.imagePosition : MISSION_INITIAL_IMAGE_POSITION')
