...
describe('CMS route wiring', () => {
  ...
  it('uses a controlled mobile destination reveal for article-card navigation without duplicate native image snapshots', () => {
    ...
    expect(article).toContain('article-hero-pending')
    ...
    expect(header).toContain('articleChromePending')
    expect(header).toContain('ARTICLE_ARRIVAL_EVENT')
    expect(header).toContain('ringSocietyArticleLoading')
    expect(header).toContain('HELLO_BAR_SESSION_KEY')
    expect(card).toContain('beginMobileArticleArrival')
    expect(article).toContain('finishMobileArticleArrival')
    expect(css).toContain("html[data-ring-society-article-loading='true'] header")
