...
describe('CMS route wiring', () => {
  ...
  it('uses a controlled mobile destination reveal for article-card navigation without duplicate native image snapshots', () => {
    ...
    expect(article).toContain('article-hero-image-arrival')
    expect(css).toContain('.article-hero-arrival')
    expect(css).toContain('.article-hero-image-arrival')
    expect(css).toContain('@keyframes article-arrival')
    expect(header).toContain('article-header-pending')
    expect(header).toContain('ring-society:article-hero-ready')
    expect(css).toContain('@keyframes article-header-arrival')
