...
describe('Top Guides loading resilience', () => {
  it('renders established fallback guide rows immediately while the CMS selection resolves', () => {
    ...
    expect(topGuides).toContain("new Set([firstGuide?.feature, firstGuide?.guideFeature].filter((source): source is string => Boolean(source)))")
    expect(topGuides).toContain('await Promise.all(firstGuideImages.map(preloadImage))')
